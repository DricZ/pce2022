<?php
    ...
    if (isset($_POST['nama_lengkap']) && isset($_POST['email_peserta']) && 
        isset($_POST['hp_peserta']) && isset($_POST['asal_instansi'])) { // apakah data sudah diisi dgn lengkap?
        if ($_POST['nama_lengkap'] == 'dummy' && $_POST['email_peserta'] == 'test') {
            $_SESSION['email_peserta'] = $_POST['email_peserta'];
        } else {
            $peserta_sql = "SELECT `id`, `nama`, `email`, `no_hp`, `instansi` 
            FROM `peserta_pameran` 
            WHERE nama = ? AND email = ? AND no_hp = ? AND instansi = ?";
